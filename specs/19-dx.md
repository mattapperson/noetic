...
3. Example code in docs MUST compile. A `typecheck:docs` script in `packages/web/package.json` MUST type-check all TypeScript code blocks:
   - A companion script at `scripts/extract-doc-snippets.ts` extracts fenced TypeScript code blocks from every `.mdx` file under `packages/web/content/` and writes them as `.ts` stub files under `packages/web/.typecheck-snippets/` (one file per block, with a shared import preamble for `@noetic-tools/core` and `@noetic/eval`).
