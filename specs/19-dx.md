...
- **Agent authors** — developers using `@noetic-tools/core` and `@noetic-tools/eval` to build agents
- **Framework extenders** — developers implementing custom context layers, agent harness backends, storage adapters, or trace exporters

### No Higher-Order "Agent" Concept

Noetic does not ship a top-level `Agent` class or `createAgent()` factory. This is intentional.

An agent is a function (or module) that the developer authors directly, composing Noetic's step primitives, context layers, channels, and context management. The framework provides the operations; the developer retains full control over the agent's structure, entry point, lifecycle, and composition strategy.

This is the same design philosophy as React: React does not ship a "Component" class that developers subclass. A component is a function that uses hooks and returns JSX. The framework provides the primitives (`useState`, `useEffect`, context) — the component is just code. Similarly, a Noetic agent is just code that calls `execute()`, configures context layers, and composes steps.
...
| Implementing a custom context layer | §6.1, `specs/11-context-layer-system` | §6.5 for publishing |
| Replacing `@noetic/context` entirely | §6.5, then §6.1–6.4 | `specs/11-context-layer-system` for full contract |
| Implementing a custom agent harness backend | §6.2, `specs/08-agent-harness` | — |
| Contributing to `@noetic-tools/core` | §1 Export Visibility, §2 API Consistency | §4 for doc standards |
...
6. Framework utilities (`frameworkCast`, `allocateBudgets`, `checkBudget`, `createLayerStateStore`, lifecycle functions) are `@internal` unless explicitly needed by framework extenders, in which case they are `@unstable` and may be exposed via a dedicated subpath export (e.g. `@noetic-tools/core/unstable`), not the root index.
...
Builder functions for the base step variants are flat top-level exports; only the coding-agent sub-harness builders stay grouped under the `step` namespace:
...
step.claudeCode(...)   // sub-harness builders keep the `step.` prefix
```
...
**Factory functions** for built-in context layers use camelCase, named after what they provide (not what they are):

```ts
scratchpad(...)   // not createScratchpad
observations(...) // not createObservations
taskState(...)    // not createTaskState
instructions(...)
steering(...)
```
...
1. All public API is importable from the package root (`@noetic-tools/core`, `@noetic-tools/eval`). Deep imports (`@noetic-tools/core/context`) are only for subpath exports explicitly declared in `package.json#exports`.
2. Subpath exports, if added, MUST follow `@noetic/<package>/<capability>` (e.g., `@noetic-tools/core/adapters`).
...
The TypeScript SDK is `@noetic-tools/core` and `@noetic-tools/eval`. "SDK" means the public API surface those packages export.
...
1. **Concept parity** — Every public primitive (`Step` variants, `execute`, context layers, `until` predicates) MUST have a first-class equivalent. Gaps are documented as known limitations with a tracking issue.
2. **Naming translation** — Naming follows target language conventions (snake_case for Python, etc.), but concept names stay semantically identical (`loop`, `inParallel`, `spawn`, `conditional`).
...
- `@throws` for every `NoeticConfigError` code the function can produce at construction time (applies to builders, factories, and context layer factories)
...
3. Example code in docs MUST compile. A `check:docs` script in `packages/web/package.json` MUST type-check all TypeScript/TSX code fences using [Kiira](https://kiira.dev):
   - `scripts/check-docs.ts` globs every `.mdx` file under `packages/web/content/` and runs them through `kiira-core`'s extract → virtual-file → type-check pipeline (Kiira's own CLI only discovers `.md`, so the runner feeds it the `.mdx` list directly).
   - Bare fences receive an injected preamble importing the full public surface of `@noetic-tools/core` (generated into `scripts/doc-preamble.ts` from `packages/core/src/index.ts`); fences that bring their own `@noetic-tools/*` imports are checked standalone. Module paths resolve to workspace source via `packages/web/tsconfig.kiira.json`.
   - Fences that document a type shape or signature, or are intentionally partial, MUST opt out with the `validate=none` fence meta rather than being contorted to compile.
...
- Introduce no more than three Noetic concepts (e.g., `callModel`, `run`, `AgentHarness`)
- Fit in under 30 lines including imports
- Be type-checkable without errors on a clean install

The canonical example is verified by the `check:docs` CI script (§4.3). Any revision MUST remain within the constraints above and MUST remain type-check clean.
...
Every error thrown during setup or configuration — including missing env vars, invalid step construction, invalid context layer config, and agent harness misconfiguration — MUST be a `NoeticConfigError`.
...
Framework extenders implement the extension interfaces defined in specs `08-agent-harness`, `11-context-layer-system`, and `10-observability`. Each extension point MUST have a dedicated getting-started guide in the docs.

### 6.1 Custom Context Layer

The context layer system is the primary advanced extension point. It is deliberately not exposed as a reactive API — users implement lifecycle hooks and the agent harness handles orchestration. Documentation and error messages for custom layers MUST NOT use reactive programming terminology (observables, subjects, streams, subscriptions). Use the framework's own vocabulary: `recall`, `store`, `slot`, `View`, `layer`, `convergence`.

A custom context layer guide MUST cover:
1. The `ContextLayer` interface (all required and optional hooks)
...
6. How to publish a custom layer as a package (peer dependency on `@noetic/context` only — `@noetic-tools/core` is not required)
...
2. The key namespacing contract (keys are owned by the context layer, not the adapter)
...
2. The `Span` and `LayerTraceSpan` types
...
Some Noetic packages are designed to be replaceable in their entirety — not just extended with a custom layer or adapter, but fully reimplemented by a third party. `@noetic/context` is the primary example: the package defines a contract, the built-in implementations ship as the default, but an alternative package (e.g. `@acme/noetic-context`) can satisfy the same contract and be swapped in.
...
2. **What the consumer expects** — how `@noetic-tools/core` (or other dependents) imports and calls into the package, so the implementor knows exactly what surface they are providing.
...
5. **The test suite** — a shared conformance test suite that any implementation can run against to verify correctness. Pluggable packages MUST ship a conformance suite importable as `@noetic/<package>/conformance`. The suite MUST cover at minimum:
   - **Lifecycle ordering** — hooks are called in the specified order (e.g. `init` before `recall`, `store` before `dispose`, `onParentUpdate` only for `spawn` not `inParallel`)
...
## 7. `@noetic-tools/eval` DX

The eval package (`@noetic-tools/eval`) is a first-class part of the Noetic SDK. Its DX standards mirror `@noetic-tools/core` unless otherwise specified.

### 7.1 CLI Install and Discovery

1. The `noetic` CLI MUST be documented as a dev dependency: `bun add -d @noetic-tools/eval`.
...
3. The `noetic-eval` command's flags (`--filter`, `--model`, `--baseline`, etc.) MUST be documented in both `--help` output and in the docs.
...
1. Every public symbol in `@noetic-tools/eval` (`describe`, `it`, `createScorer`, `runEval`, `EvalResult`, etc.) MUST have TSDoc with the same requirements as `@noetic-tools/core` public symbols (see §4.1).
...
| `allocateBudgets` | `@internal` or `@unstable` | Layer budget internals |
| `checkBudget` | `@internal` or `@unstable` | Layer budget internals |
| `findFunctionCall` | `@internal` | Internal layer utility |
...
| `assembleView` | `@internal` or `@unstable` | Layer projector internals |
...
| No framework extender guides (custom context layer, agent harness, storage adapter, trace exporter) | §6.1–6.4 |
| `@noetic-tools/eval` public symbols have no TSDoc | §4.1, §7.2 |
...
| No `@noetic-tools/core/internal` subpath export; internal symbols exposed at root index without `@internal` tags | §1.6 |
