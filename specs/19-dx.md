...
1. All public API is importable from the package root (`@noetic-tools/core`, `@noetic-tools/eval`). Deep imports (`@noetic-tools/core/memory`) are only for subpath exports explicitly declared in `package.json#exports`.
...
6. How to publish a custom layer as a package (peer dependency on `@noetic/memory` only — `@noetic-tools/core` is not required)
