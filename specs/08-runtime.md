...
|----------------------------|-----------------------------------------------------------------|-----------------|
| `InMemoryAgentHarness`     | Testing, simple scripts, CLI tools. Auto-detects `callModel` from `OPENROUTER_API_KEY` when none is provided. | In-process handles |
...
setHarness(new InMemoryAgentHarness({ callModel: createOpenRouterCallModel() }));
