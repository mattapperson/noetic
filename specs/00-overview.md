...
- **`@noetic-tools/system-one`** — A client for System One models: inference that returns typed, probabilistic decisions (`noul` / `choice` / `score`) instead of text. A thin adapter over the vendor's official `@typesafe-ai/sdk`, exposing the provider factories (`jev()` for hosted, `openSystemOne()` for any self-hosted endpoint speaking the same wire format) behind Noetic's vendor-neutral contract. Depends on `@noetic-tools/types` plus that SDK — and `@noetic-tools/core` must never import it, so an agent that composes `step.decide` without constructing a client pulls in no transport. See `32-system-one-decisions`.
