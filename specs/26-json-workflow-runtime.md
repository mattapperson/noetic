...
- The artifact is committed at `packages/core/schema/noetic-workflow.schema.json` and exposed at the `@noetic-tools/core/schema` export subpath. Its stable `$id` is `https://noetic.tools/schema/noetic-workflow.schema.json`.
- It is generated by `bun run gen:schema` (`scripts/generate-workflow-schema.ts`), which calls `z.toJSONSchema(WorkflowDocumentSchema, { target: 'draft-2020-12' })`. The two recursive types — `WorkflowNode` and `UntilPredicate` — are emitted as named `$defs`; every other subschema is inlined.
- A drift-gate test re-serialises the schema and asserts the committed file is byte-for-byte current, and that the published variants cover exactly the runtime node and predicate kinds. CI fails if `gen:schema` was not re-run after a schema change.
