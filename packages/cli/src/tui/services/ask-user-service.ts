 ...
 *
 * Tool code calls `request(input)` and awaits a Promise. The TUI subscribes
 * via `subscribe()`, renders the modal when a request lands, and resolves via
 * `resolve(output)` or `cancel(reason)` when the user submits/dismisses.
 *
 * Only one concurrent request is permitted per service — a second `request()`
 * while one is pending throws `AskUserBusyError`. Cancellation is reported as
 * a `NoeticErrorImpl({ kind: 'cancelled' })` so the harness's error pipeline
 * maps it to the canonical `cancelled` runtime error.
 */
...
export interface AskUserService {
  /** Enqueue a request; resolves when the TUI calls `resolve()`, rejects on `cancel()`/`cancelAll()`. */
  ...
  /**
   * Cancel any currently-pending request without needing the id. Used by the
   * TUI on harness swaps, user-stop, and session reset so a ghost dialog
   * can't outlive the harness that produced it.
   */
