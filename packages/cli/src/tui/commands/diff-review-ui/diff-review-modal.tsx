...
type BrowseInputKey = Parameters<Parameters<typeof useInput>[0]>[1];

function useBrowseKeymap(deps: KeymapDeps): void {
  useInput((input, key) => {
    handleBrowseKey(input, key, deps);
  });
}

function handleBrowseKey(input: string, key: BrowseInputKey, deps: KeymapDeps): void {
  if (deps.state.mode !== Mode.Browse) {
    return;
  }
  clearToastOnKeypress(deps);
  if (handleBrowseExit(input, key, deps)) {
    return;
  }
  if (handleBrowseScopeKey(key, deps)) {
    return;
  }
  if (handleBrowsePaneKey(input, key, deps)) {
    return;
  }
  if (handleBrowseMoveKey(input, key, deps)) {
    return;
  }
  handleBrowseActionKey(input, key, deps);
}

function clearToastOnKeypress({ state, dispatch }: KeymapDeps): void {
  if (state.toast === null) {
    return;
  }
  dispatch({
...
function handleBrowseExit(input: string, key: BrowseInputKey, deps: KeymapDeps): boolean {
...
function handleBrowseScopeKey(key: BrowseInputKey, { state, dispatch }: KeymapDeps): boolean {
  if (!key.tab) {
    return false;
  }
  dispatch({
    type: 'set-scope',
    scope: nextScope(state.scope, key.shift ? -1 : 1),
  });
  return true;
}

function handleBrowsePaneKey(input: string, key: BrowseInputKey, deps: KeymapDeps): boolean {
...
function handleBrowseMoveKey(input: string, key: BrowseInputKey, deps: KeymapDeps): boolean {
  ...
  const next = Math.max(0, Math.min(len - 1, deps.state.cursorIndex + delta));
  deps.dispatch({
    type: 'set-cursor-index',
    index: next,
  });
  if (deps.state.activePane === Pane.Files) {
...
function handleBrowseActionKey(input: string, key: BrowseInputKey, deps: KeymapDeps): void {
  if (key.return && deps.state.activePane === Pane.Files) {
    handleListEnter(deps);
    return;
  }
  if (input === 'c') {
    handleAddComment(deps);
    return;
  }
  if (input === 'O') {
    deps.dispatch({
  ...
  if (input === 'd') {
    handleDeleteFirstCommentOnLine(deps);
    return;
  }
  if (input === 's') {
    deps.onConfirmSubmit();
  }
}
