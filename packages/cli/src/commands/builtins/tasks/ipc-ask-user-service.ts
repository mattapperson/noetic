 ...
 *
 * The TUI's in-memory ask-user service (`tui/services/ask-user-service.ts`)
 * runs in the same process as its modal renderer. Headless task runners
 * (planner / implementer / validator subprocesses) have no in-process UI
 * — their human counterpart is whatever client is currently connected to
 * the per-task IPC socket. This service implements the same `AskUserService`
 * contract by routing `request()` to a `broadcast` callback (provided by
 * the IPC server) and accepting answers/cancellations through
 * `handleResolve` / `handleCancel` methods that the IPC server invokes
 * when client frames arrive.
 *
...
} from '../../../tui/services/ask-user-service.js';
import {
  AskUserBusyError,
  createAskUserCancelledError,
} from '../../../tui/services/ask-user-service.js';
