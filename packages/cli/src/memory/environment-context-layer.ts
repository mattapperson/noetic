...
async function detectGitRepository(shell: ShellAdapter, cwd: string): Promise<{ isRepo: boolean; branch?: string }> {
  try {
...
async function detectNodeVersion(shell: ShellAdapter, cwd: string): Promise<string> {
  try {
    const result = await shell.exec('node --version', { cwd, timeout: 5 });
...
async function detectShellType(shell: ShellAdapter, cwd: string): Promise<string> {
  try {
    ...
    const result = await shell.exec('echo $SHELL', { cwd, timeout: 3 });
    if (result.exitCode === 0) {
...
async function detectPackageManager(shell: ShellAdapter, cwd: string): Promise<'npm' | 'yarn' | 'pnpm' | 'bun' | undefined> {
  const checkFile = async (filename: string): Promise<boolean> => {
    try {
      const result = await shell.exec(`test -f "${filename}"`, { cwd, timeout: 3 });
...
- **Available commands**: ${env.availableCommands.filter(cmd => 
      ['npm', 'yarn', 'pnpm', 'bun'].includes(cmd)
    ).join(', ') || 'none detected'}`);
  }
...
export function environmentContextLayer(config: EnvironmentContextConfig): MemoryLayer<EnvironmentContextState> {
  return {
