...
/** Validator failure summary that, when present, switches triage into Fix-task mode. */
...
/** Argument bag for {@link buildTriageUserPrompt}. */
...
/** Argument bag for {@link buildValidationSystemPrompt}. */
...
/**
 * System prompt for the live planning interview. Drives the AI through a
 * Q&A loop and asks it to emit a JSON object matching the
 * `TaskHierarchyInput` shape (milestones[].slices[].features[]) at completion.
 *
 * The completion envelope mirrors `TaskHierarchyInput` from
 * `tasks/hierarchy/schemas.ts` (top-level title/description live on the
 * parent Task itself, not in this tree input).
 */
export const INTERVIEW_SYSTEM_PROMPT = `You are a task planning assistant for a project management system.

Your job: help users transform high-level goals into structured task plans with milestones, slices, and features — each with verification criteria.

## Task Hierarchy
- Task: The top-level objective (the user will provide this; title/description belong on the parent task itself)
- Milestone: A major phase or deliverable within the task. Each milestone has verification criteria that define how to confirm the phase is complete.
...
- Feature: A specific deliverable within a slice, detailed enough to become a leaf task. Each feature has acceptance criteria.

## Conversation Flow
1. The user describes their task goal
...
For completion (the "milestones" array is the canonical TaskHierarchyInput shape):
{"type": "complete", "data": {"milestones": [{"title": "...", "description": "...", "verification": "...", "slices": [{"title": "...", "description": "...", "verification": "...", "features": [{"title": "...", "description": "...", "acceptanceCriteria": "..."}]}], "assertions": [{"title": "...", "assertion": "...", "featureIndices": [0, 1]}]}]}}

The "acceptanceCriteria" field is a single string capturing the concrete, testable acceptance bar for the feature. Aim for 2-4 milestones, 1-3 slices per milestone, 2-5 features per slice. The "assertions" array on each milestone may be empty.`;

//#endregion

//#region Triage prompt

export const TRIAGE_SYSTEM_PROMPT = `You are a task specification agent for "noetic tasks". Your job: take a feature spec and produce a fully specified PROMPT.md that an autonomous coding agent can execute end-to-end without further human guidance.
...
/** Build the per-feature user prompt that pairs with {@link TRIAGE_SYSTEM_PROMPT}. */
...
export function buildValidationSystemPrompt(args: ValidationSystemPromptArgs): string {
  ...
  const sections: string[] = [];
  sections.push(
    'You are a strict, read-only validator for the noetic tasks framework.',
