...
import type { ContextLayer, LayerStateStore } from '@noetic-tools/context';
import {
  completeLayers,
...
): ContextLayer<CounterState | undefined> {
  return {
    id: 'counter',
    slot: 100,
    scope,
...
interface InitOnParams {
  layer: ContextLayer<CounterState | undefined>;
...
describe('M1: durable write-through via LayerStateStore.set()', () => {
  ...
  it('onComplete-returned state is durably persisted', async () => {
    const storage = makeStorage();
    const layer: ContextLayer<CounterState | undefined> = {
      ...counterLayer(),
      hooks: {
        ...counterLayer().hooks,
