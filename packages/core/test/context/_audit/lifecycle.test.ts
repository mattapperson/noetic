/**
 * REGRESSION SUITE — context layer lifecycle orchestrator.
 *
...
function budgetsFor(ls: ContextLayer[]): Map<string, number> {
...
describe('AUDIT-1 budget allocator (allocateBudgets wired into recall)', () => {
  function budgetMap(layers: ContextLayer[], totalBudget: number): Map<string, number> {
    const { allocations } = allocateBudgets({
      layers,
      totalBudget,
      systemPromptTokens: 0,
  ...
  it('1b: N "auto" layers stay BOUNDED by the context window', () => {
    const layers: ContextLayer[] = Array.from(
      {
        length: 100,
      },
      (_, i) => ({
...
describe('AUDIT-4 init-failure policy', () => {
  function failingLayer(onInitError?: 'throw' | 'disable'): ContextLayer {
    return {
      id: 'transient',
      slot: 100,
      scope: 'thread',
...
describe('AUDIT-EXTRA', () => {
  ...
  it('E5: recall slot-sort is STABLE for equal slots (insertion order preserved) — no bug', async () => {
    const store = createLayerStateStore();
    const order: string[] = [];
    const mk = (id: string): ContextLayer => ({
      id,
      slot: 100,
      scope: 'execution',
      hooks: {
