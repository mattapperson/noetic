 ...
 * implement HTTP, MCP, or authorization take precedence over configured headers
 * with the same case-insensitive name."
 *
 * The precedence has to be enforced here rather than left to the SDK, which
 * merges the wrong way round — `new Headers({ ...generated, ...extraHeaders })`
 * puts the plugin's value last, so a plugin could replace the agent's bearer
 * token or graft itself onto another server's session.
 */
...
/**
 * Remove plugin-configured headers the client owns. Matching is
 * case-insensitive because HTTP field names are.
 *
 * @public
 */
export function stripClientOwnedHeaders(headers: Record<string, string>): Record<string, string> {
...
/** @public Which configured headers {@link stripClientOwnedHeaders} would drop. */
export function findClientOwnedHeaders(headers: Record<string, string>): DroppedHeader[] {
  return Object.keys(headers)
    .filter((name) => CLIENT_OWNED_HEADERS.has(name.toLowerCase()))
    .map((name) => ({
      name,
...
async function createTransport(params: {
  ...
  const safeHeaders = stripClientOwnedHeaders(server.headers);
  const requestInit =
