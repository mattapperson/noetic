...
| subAgentMiddleware | `createConfigurableDelegateTool()` using `toolCtx.runtime` |
...
The `createConfigurableDelegateTool` uses `toolCtx.runtime` to execute sub-agents — no need to pass the runtime at construction time:
...
```ts
export function buildDeepAgent(config): StepLoop | StepSpawn {
  const allTools = [...fsTools, ...todoTools, delegateTool];
  const layers = [
    staticContent({ load: ..., tag: 'instructions' }),
