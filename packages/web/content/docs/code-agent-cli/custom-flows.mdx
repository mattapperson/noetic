...
When the agent is in **plan mode** (`/plan` or `/mode plan`), it authors the plan as a `WorkflowDocument` — the same format the [JSON Workflow Runtime](/docs/framework/json-runtime) executes, validated against the published schema at `https://noetic.tools/schema/noetic-workflow.schema.json`. The [plan memory layer](/docs/framework/memory/plan-memory) stores the reviewed tree via `plan/setPlanTree` plus any **named workflows** via `plan/setWorkflow`, and the tree references them with `subflow` nodes so the part a human approves stays small.
