...
| `trustProjectEmbeddedCommands` | `boolean` | `false` | If `true`, project-origin AGENT.md/rules files and project-origin skills execute `!command` lines (at session start and on skill activation respectively). User-origin files (`~/...`) and user/built-in/plugin skills always execute commands. Off by default for supply-chain safety. All executed embedded commands pass the Bash tool's preflight (command validation + mutation policy). |
...
| `context` | `string[]` | — | Identifiers for additional context layers to install (advanced). |
...
| `/context` | Show context window breakdown by context layer (from last run) |
