...
The file is loaded by the `agent-md` context layer, which sits just above observations in the [context budget](/docs/framework/context-layers). Total budget is **15 KB recall**; per-file content cap is 25 KB; aggregate cap across all sources is 60 KB. When the aggregate cap is exceeded, lower-precedence sources are dropped and a note is appended.
