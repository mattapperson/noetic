...
| `agent-max-steps` | `number` | Cap reasoning steps for tool-bearing teammates (enforced via `until.maxSteps` on the agent loop). |
...
Activation is per-thread. The `skills` context layer tracks which skills are active and renders their processed instructions on every recall.
