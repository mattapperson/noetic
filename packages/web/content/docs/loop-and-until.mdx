...
```ts
import { step, until } from '@orchid/core';
...
| `onError` | `(error: OrchidError, ctx: Context) => 'retry' \| 'skip' \| 'abort'` | No | -- | Handle errors from the body step |
...
Orchid ships with several ready-made predicates on the `until` object.
...
Stop when the output stabilizes (consecutive outputs are identical).

```ts
const predicate = until.converged({});
```
...
```ts
import { any, until } from '@orchid/core';
...
```ts
import { all, until } from '@orchid/core';
...
```ts
import { any, step, until } from '@orchid/core';
