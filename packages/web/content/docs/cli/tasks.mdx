...
### Run autopilot on a structured task

Autopilot is a per-task flag the daemons consult to decide whether to triage features, spawn validator runs, and generate fix features automatically.

```bash
noetic tasks autopilot on <T-id>
# → flips autopilotEnabled=true; the daemon attaches on its next health tick

noetic tasks activate-slice --task <T-id> --slice <SL-id>
# → marks the slice 'active' and triages its features into leaf tasks under
#   the same parent T-id, each labelled [w] in the kanban
```

Flip it back off any time:
