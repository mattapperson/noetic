...
const harness = new AgentHarness({ name: 'example', params: {} });
...
The AgentHarness is the execution engine at the heart of Noetic. It creates contexts, dispatches steps, manages channels and memory layers, and provides tracing infrastructure. You can use the built-in `AgentHarness` or implement the `AgentHarness` interface for durable, distributed, or custom execution environments.
...
`AgentHarness` is the built-in implementation. It keeps all state in memory -- no persistence, no external dependencies.
