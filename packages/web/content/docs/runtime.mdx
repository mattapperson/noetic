---
title: Runtime
description: The Runtime interface executes steps, manages context, and coordinates memory and channels.
---

## Quick Example

```ts
import { InMemoryRuntime, step } from '@orchid/core';
...
The Runtime is the execution engine at the heart of Orchid. It creates contexts, dispatches steps, manages channels and memory layers, and provides tracing infrastructure. You can use the built-in `InMemoryRuntime` or implement the `Runtime` interface for durable, distributed, or custom execution environments.
...
| `getChannelHandle(channel, executionId)` | Get a [`ChannelHandle`](/docs/channels#channelhandle) for an external channel, enabling outside processes to push values in. |

### Memory Methods

The runtime coordinates [memory layer](/docs/memory) lifecycle hooks:
...
```ts
import { InMemoryRuntime, InMemoryExporter } from '@orchid/core';
...
- [Context & Event Log](/docs/context) -- the context object the runtime creates.
- [Channels](/docs/channels) -- channel send/recv methods on the runtime.
- [Memory](/docs/memory) -- memory layers managed by the runtime.
