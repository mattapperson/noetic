---
title: Step Types
description: Type definitions for all step variants in Orchid's discriminated union.
---
...
| `contextIn` | `ContextInStrategy` | yes | How the child inherits context |
| `contextOut` | `ContextOutStrategy<O>` | yes | How the child's output is extracted |
...
| `onError` | `(error: OrchidError, ctx: Context) => 'retry' \| 'skip' \| 'abort'` | no | Error handler |
...
| `error` | `OrchidError` | Present if rejected |
