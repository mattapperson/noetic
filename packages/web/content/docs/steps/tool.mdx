...
Every tool in Orchid follows the same interface, whether it is passed to `step.llm` or invoked directly via `step.tool`.
...
import type { Tool } from '@orchid/core';
...
- [step.llm](/docs/steps/llm) -- pass tools to an LLM and let the model decide when to call them.
- [step.run](/docs/steps/run) -- for arbitrary async logic that is not a tool.
- [Steps overview](/docs/steps) -- comparison of all three step types.
