...
Tools passed to `step.llm` follow the standard Orchid `Tool` interface. See [step.tool](/docs/steps/tool) for the full definition.
...
When the model emits a tool call, the runtime executes the tool and feeds the result back. To loop until the model stops calling tools, wrap the LLM step in a [loop](/docs/loop-and-until) with `until.noToolCalls()`.
...
- [Loop & Until](/docs/loop-and-until) -- wrap an LLM step in a ReAct-style loop.
