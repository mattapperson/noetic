...
2. **ctx** -- the Orchid `Context`, giving access to memory, tokens, abort signals, and more.
...
| `execute` | `(input: I, ctx: Context) => Promise<O>` | Yes | The async function to run |
...
- [Loop & Until](/docs/loop-and-until) -- use `step.run` as a loop body for custom iteration logic.
