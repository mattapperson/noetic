---
title: Steps
description: The three step primitives -- run, llm, and tool -- are the atomic units of work in Orchid.
---

Steps are the building blocks of every Orchid agent. Each step takes typed input, does one thing, and returns typed output. There are three kinds.
...
See [step.run deep dive](/docs/steps/run).
...
See [step.llm deep dive](/docs/steps/llm).
...
See [step.tool deep dive](/docs/steps/tool).
...
- [Control Flow](/docs/control-flow) -- route between steps with `branch` and `fork`.
- [Loop & Until](/docs/loop-and-until) -- repeat steps with termination conditions.
- [Spawn](/docs/spawn) -- run steps in isolated child contexts.
