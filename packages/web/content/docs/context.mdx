...
Every step in Orchid receives a `Context` object as its second argument. The context carries execution metadata, token budgets, the conversation item log, channel methods, and lifecycle controls. It is the single source of truth for everything that has happened during a run.
...
Context exposes three methods for communicating over [Channels](/docs/channels):
...
- [Runtime](/docs/runtime) -- creates and manages contexts.
- [Channels](/docs/channels) -- typed messaging between steps.
