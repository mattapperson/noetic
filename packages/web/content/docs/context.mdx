...
For the complete reference of all item shapes, streaming events, and how they map to the OpenResponses specification, see [Items & Events](/docs/api/items-and-events).
...
See [Context Types](/docs/api/context-types) for the `LastLayerUsage` and `LayerUsageEntry` interfaces.
