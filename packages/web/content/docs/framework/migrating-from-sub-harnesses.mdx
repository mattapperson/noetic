...
opencode speaks ACP natively — the preset runs its official `opencode acp` command. pi has no native ACP mode yet, so its preset goes through the community [`pi-acp`](https://github.com/svkozak/pi-acp) adapter — the same bridge Zed's agent registry points at. If the pi preset misbehaves, the issue likely belongs in that adapter rather than in pi or Noetic.
