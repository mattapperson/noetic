...
The standalone `assembleView()` function (from `context/projector.ts`) merges system-prompt, layer `recall` output, and conversation-history `Item` objects into a single ordered list that is sent to the LLM. The runtime calls it for you on every turn — you rarely need it directly.
