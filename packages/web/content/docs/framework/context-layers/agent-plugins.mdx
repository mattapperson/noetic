...
Because only activation changes the block, the layer implements `renderDelta`: a newly activated skill is published as a delta rather than rewriting the whole cached prefix.
...
- Plugin subprocesses receive a narrow environment allowlist, not the agent's environment. Ambient secrets — API keys and the like — are not passed through. `PLUGIN_ROOT` and `PLUGIN_DATA` are applied last, so a plugin cannot spoof them.
