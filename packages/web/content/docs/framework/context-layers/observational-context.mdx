...
Observational Context accumulates conversation text into a buffer, and when the buffer crosses a token threshold, distills the content into a list of observations. These observations persist across turns and are injected as an `<observations>` block in the prompt.
...
```ts validate=none
interface ObservationalContextConfig {
