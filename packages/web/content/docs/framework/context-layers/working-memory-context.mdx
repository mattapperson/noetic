...
Working Memory is the simplest built-in context layer. It acts as a scratchpad that the agent can read and write each turn. The LLM sees the current state as a `<working_memory>` block in the prompt, and updates it by calling an `updateWorkingMemory` tool.
...
```ts
import { workingMemoryContext } from '@noetic-tools/core';
...
```ts validate=none
interface WorkingMemoryContextConfig {
...
```ts validate=none
type WorkingMemoryContextState = string | Record<string, unknown>;
```
...
Working Memory exposes two entries via the `provides` API, making them accessible on `ctx.context['working-context']`:
...
```ts validate=none
const current = ctx.context['working-context'].snapshot;
// Returns WorkingMemoryContextState (string | Record<string, unknown>)
```
...
```ts validate=none
await ctx.context['working-context'].update({ currentGoal: 'Ship v2' });
```
