...
```ts validate=none
import { temporal } from '@noetic-tools/core';
...
The layer provides one `layerFunction`, exposed to the model as the `temporal/searchMemory` tool and to code as `ctx.context['temporal'].searchMemory({ query })`:
...
The layer is LLM-agnostic: `extract` and `search` are host-injected (mirroring `observations`'s `observer`), keeping the context package tree-shakable. The code agent wires structured `callModel` calls as the callbacks and installs `temporal()` in its default stack.
