...
The layer is LLM-agnostic: `extract` and `search` are host-injected (mirroring `observations`'s `observer`), keeping the context package tree-shakable. The code agent wires structured `step.callModel` calls as the callbacks and installs `temporal()` in its default stack.
