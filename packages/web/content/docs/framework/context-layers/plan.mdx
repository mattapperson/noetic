...
`plan/setPlanTree` stores a `WorkflowDocument` — the same portable JSON format the [JSON Workflow Runtime](/docs/framework/json-runtime) executes, with node kinds like `callModel`, `invokeTool`, `sequence`, `inParallel`, `conditional`, `loop`, `spawn`, `subflow`, and the coding-agent kinds (`claude-code`, `codex`, `opencode`, `pi`).
