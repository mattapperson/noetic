...
`maxItems` is validated at construction time: it must be an integer in `[2, 10000]`, otherwise `history()` throws a `NoeticConfigError` with code `INVALID_HISTORY_WINDOW_MAX_ITEMS`.
