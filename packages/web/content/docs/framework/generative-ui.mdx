...
Install `openUiSurface()` as a context layer to make the server the owner of UI state. It is modeled on [`durableTaskState`](/docs/framework/context-layers) (thread-scoped, durable, rendered into the view) and the [steering layer](/docs/framework/context-layers) (enforcement hooks).
