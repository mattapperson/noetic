...
Non-Zod validators are validated through the Standard Schema `~standard.validate` hook (sync or Promise). The model wire boundary, however, is Zod-bound: `@openrouter/agent` expects Zod input schemas and runs `z.toJSONSchema` itself. Rather than take a runtime dependency on each validator's JSON-Schema converter (or on `@standard-community/standard-json`, whose converter peers and maturity don't meet the bar), Noetic bridges that boundary with a self-contained handoff: the user supplies the raw JSON Schema explicitly (`inputJsonSchema` on tools, `outputJsonSchema` on `step.llm`), and it is passed through untouched. No per-validator runtime dependencies; missing escape hatches fail fast with `NoeticConfigError` code `MISSING_JSON_SCHEMA`.
