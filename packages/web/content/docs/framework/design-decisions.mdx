...
Because context flow is just layers, the same mechanism scales from "fully fresh child" (no layers) to selective history injection, summarized hand-backs, and typed state merging — enabling compositions like a fresh context per iteration, or a fresh child whose result is distilled before it returns. See [Spawn](/docs/framework/operators/spawn) for the full mechanics.
