...
Returns `null` when no snapshot is recorded. Throws `NoeticConfigError` with `code: 'CHECKPOINT_SCHEMA_MISMATCH'` when the persisted `schemaVersion` is unknown — callers discard via `harness.clearCheckpoint(executionId)` and start fresh. Schema v1 snapshots (written before the context-layer rename) fall in this bucket: their `layers` record is keyed by the old `working-memory` / `observational-memory` ids, which no current layer reads, so they are rejected rather than restored with silently missing state. There is no migration path.
...
Durable execution can replay a step body whose prior completion's checkpoint failed to land. The framework cannot make arbitrary `step.runCode` bodies idempotent — write bodies that are safe to re-execute where durability matters, or gate with an external idempotency key.
...
Per-step overrides work the same way: set `subprocess: localAdapter` on a `step.runCode` or `spawn` opts and every dispatch of that step uses the local adapter regardless of the harness default.
