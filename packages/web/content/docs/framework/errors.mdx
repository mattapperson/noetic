...
| `cancelled` | `reason?: string` | The execution was cancelled via `ctx.abort()` or `harness.cancel()`. |
...
const resilientLoop = loop({
  id: 'retry-loop',
  steps: [step.llm({ id: 'generate', model: 'gpt-4o' })],
...
- [AgentHarness](/docs/runtime) -- `harness.cancel()` triggers cancellation errors.
