...
Configuration for the LLM provider, passed to `AgentHarness` via the `callModelDefaults` option.
...
```ts
import { callModel, conditional, createOpenRouterEmbed, semanticSwitch } from '@noetic-tools/core';
