...
| `outputJsonSchema` | `Record<string, unknown>` | no | Raw JSON Schema sent to the model as the structured-output constraint. Required for non-Zod `output` (`MISSING_JSON_SCHEMA` otherwise); ignored for Zod |
...
| `inputJsonSchema` | `Record<string, unknown>` | Optional raw JSON Schema sent to the model for this tool's input. Required when `input` is not a Zod schema and the tool is exposed to a model — conversion throws `NoeticConfigError` code `MISSING_JSON_SCHEMA` otherwise. Ignored for Zod inputs, whose JSON Schema is derived via `z.toJSONSchema`. |
