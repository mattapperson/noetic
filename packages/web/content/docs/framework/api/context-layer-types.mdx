...
An **omitted** `budget` behaves like `'auto'`: the layer gets the same fixed 2000-token default cap as an explicit `'auto'` budget. Declare a number or `{ min, max }` when a layer needs more.
