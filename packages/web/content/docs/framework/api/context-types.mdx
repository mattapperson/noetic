...
| `abort` | `(reason?) => void` | Signal abort. Cascades to every live `inParallel` path and `spawn` child of this context (never to the parent); first call wins. Does not run context-layer teardown — see `harness.cancel` |
