...
```ts
import { callModel, spawn } from '@noetic-tools/core';
...
```ts
import { callModel, createMessage, spawn } from '@noetic-tools/core';
...
```ts validate=none
import { callModel, createMessage, loop, spawn, until } from '@noetic-tools/core';
import type { ContextLayer } from '@noetic-tools/core';
...
```ts
import { callModel, channel, spawn, tool, until } from '@noetic-tools/core';
...
const delegateTool = tool({
  ...
  execute: async (args: { task: string }, toolCtx) => {
    const child = callModel<ContextData, string, string>({
...
Every `runCode`, `spawn`, and `harness.detachedSpawn` dispatches through a `SubprocessAdapter`. The harness defaults to `createInMemorySubprocessAdapter()` so zero-config callers keep their existing in-process behaviour; swap in a different adapter to change how a step actually runs.
...
const isolated = spawn({
  id: 'sub-agent',
  child: callModel({ id: 'researcher', model: 'openai/gpt-4o' }),
...
When the interpreter dispatches a `runCode` or `spawn`, it resolves the adapter in this order:

1. **Per-call override** — the `overrides.subprocess` argument to `harness.detachedSpawn(step, input, ctx, overrides)`.
2. **Per-step override** — the `subprocess` field on `StepRunCode` or `StepSpawn`.
3. **Harness default** — `harness.subprocess` (defaults to `createInMemorySubprocessAdapter()`).

Other step kinds (`callModel`, `invokeTool`, `conditional`, `inParallel`, `withContext`, `loop`) always use the harness default.
...
## withContext vs spawn

Use [`withContext`](/docs/framework/operators/with-context) when you want to attach context layers to a subtree without isolating the child context. The child shares the parent's `ItemLog` and conversation history -- layers simply become available to all descendant steps.
...
- [withContext](/docs/framework/operators/with-context) -- attach context layers without context isolation.
- [inParallel](/docs/framework/operators/in-parallel) -- parallel execution without context isolation.
