...
const contextLayer: MemoryLayer<ResearchState> = {
  id: 'context-layer',
...
const briefingLayer: MemoryLayer<BriefingState> = {
...
const delegateTool = tool({
  ...
  execute: async (args: { task: string }, toolCtx) => {
    const child = step.llm<ContextMemory, string, string>({
      id: 'sync-sub', model: 'gpt-4o', instructions: 'Answer concisely.',
    });
...
const launchTool = tool({
  ...
  execute: async (args: { task: string }, toolCtx) => {
    ...
    handle.await().then(
      (result) => toolCtx.harness.send(inbox, `[Agent ${handle.id} done] ${result}`, toolCtx.ctx),
      (err) => toolCtx.harness.send(inbox, `[Agent ${handle.id} failed] ${err}`, toolCtx.ctx),
    );
...
Use [`provide`](/docs/framework/operators/provide) when you want to attach memory layers to a subtree without isolating the child context. The child shares the parent's `ItemLog` and conversation history -- layers simply become available to all descendant steps.
...
- [provide](/docs/framework/operators/provide) -- attach memory layers without context isolation.
...
- [Overview](/docs/framework/overview) -- how spawn fits into the seven primitives.
