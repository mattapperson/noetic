...
Each path is a child execution. It inherits the parent's context layers and tool pool, so `llm` steps inside a path keep their context projection and layer tools, and a nested `spawn` has layers to inherit.
...
For layers that collect artifacts from many workers, choose a merge strategy that namespaces per child -- see [`durableTaskState({ mergeData: 'namespace' })`](/docs/framework/context-layers/durable-task-state).
