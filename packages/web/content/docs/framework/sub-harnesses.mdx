...
| `output` | `StandardSchemaV1<unknown, O>` | Optional output schema (any Standard Schema v1; Zod is the default). When set, the agent's final text is JSON-parsed and validated, exactly like `step.llm`'s `output`. Unlike `step.llm`, no JSON Schema is sent to a model, so non-Zod schemas need no escape hatch. |
