...
A sub-harness step delegates a turn to an external coding-agent runtime, exactly the way [`step.callModel`](/docs/framework/steps) delegates a turn to a language model. You pick the agent with a builder (`step.claudeCode`, `step.codex`, `step.opencode`, `step.pi`) and hand it an adapter from the matching package:
...
| `output` | `ZodType<O>` | Optional Zod schema. When set, the agent's final text is JSON-parsed and validated, exactly like `step.callModel`'s `output`. |
...
A sub-harness sees the conversation so far, not just its own prompt — so a coding agent run *after* `step.callModel` steps (or on a later turn of a consecutive [`harness.execute()`](/docs/framework/runtime) conversation) has full context and doesn't act confused.
...
const plan = step.callModel({ id: 'plan', model, instructions: 'Note the deploy policy.' });
