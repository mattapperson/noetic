...
A sub-harness step delegates a turn to an external coding-agent runtime, exactly the way [`callModel`](/docs/framework/steps) delegates a turn to a language model. You pick the agent with a builder (`step.claudeCode`, `step.codex`, `step.opencode`, `step.pi`) and hand it an adapter from the matching package:
...
const review = step.claudeCode<ContextData, string, string>({
  id: 'review',
...
const harness = new AgentHarness({ name: 'reviewer', agentGraph: review, params: {} });
...
A *sub-harness* is a pluggable backend that drives an agentic coding tool as a step. It is the direct analogue of a [context layer](/docs/framework/context-layers): a contract defined in the dependency-free `@noetic-tools/types` foundation, implemented once per tool in its own package, and consumed by `@noetic-tools/core` without ever forming an import cycle.
...
step.claudeCode<TContext = ContextData, I = unknown, O = unknown>({
  id: string;
  harness: SubHarness | ((ctx: Context<TContext>) => SubHarness); // the adapter
  prompt: string | ((ctx: Context<TContext>) => string);         // the turn prompt
  settings?: SubHarnessSettings;
  instructions?: string | ((ctx: Context<TContext>) => string | undefined);
...
| `output` | `StandardSchemaV1<unknown, O>` | Optional output schema (any Standard Schema v1; Zod is the default). When set, the agent's final text is JSON-parsed and validated, exactly like `callModel`'s `output`. Unlike `callModel`, no JSON Schema is sent to a model, so non-Zod schemas need no escape hatch. |
...
- `reuse` keys a live session (workspace + conversation history + running runtime) that survives across steps. Two steps with the same `reuse` key share one session, so the second turn sees the first turn's history. Concurrent first turns on one key dedupe onto a single session start. A reused session is kept alive by default.
...
A sub-harness sees the conversation so far, not just its own prompt — so a coding agent run *after* `callModel` steps (or on a later turn of a consecutive [`harness.execute()`](/docs/framework/runtime) conversation) has full context and doesn't act confused.
...
```ts validate=none
// An earlier callModel step established a fact in the conversation…
const plan = callModel({ id: 'plan', model, instructions: 'Note the deploy policy.' });
...
A sub-harness's output is mapped onto the *same* event surface as a callModel step's, so you observe a coding agent exactly like you observe a model — its text, reasoning, and tool calls flow through the runtime streams:
...
### Turn failure and the idle watchdog

A turn the agent reports as failed (`finishReason: 'error'`) fails the step with a `step_failed` error — partial output is kept in the item log as evidence, but it is never returned as a successful result. And because an external coding agent is a real process that can hang, each turn is guarded by an idle watchdog: if no stream part arrives for `settings.extra.idleTimeoutMs` (default 120s, `0` disables), the turn is aborted and the step fails instead of hanging the run.

Reasoning deltas are retained as a `reasoning` item in the turn result, so the item log, checkpoints, and eval scorers see the agent's thinking.
...
- [Context Layers](/docs/framework/context-layers) — the contract this design mirrors.
