...
`step.callModel` calls a language model. You provide the model name and optionally a system prompt, tools, structured output schema, and generation parameters. The interpreter handles message formatting, tool execution loops, and response parsing.
...
Tools passed to `step.callModel` follow the standard Noetic `Tool` interface. See [step.invokeTool](/docs/framework/steps/invoke-tool) for the full definition.
...
const researcher = step.callModel({
  id: 'research',
  model: 'openai/gpt-4o',
...
- [step.runCode](/docs/framework/steps/run-code) -- custom async logic.
- [step.invokeTool](/docs/framework/steps/invoke-tool) -- invoke a tool directly without an LLM.
