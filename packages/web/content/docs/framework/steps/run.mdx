...
The `ctx.memory` property provides typed access to data and functions exposed by memory layers via their `provides` field. See [Context](/docs/context) for the full property reference.

## Related

- [step.llm](/docs/steps/llm) -- for language model calls.
- [step.tool](/docs/steps/tool) -- for direct tool invocation.
- [Loop & Until](/docs/operators/loop-and-until) -- use `step.run` as a loop body for custom iteration logic.
