...
| `inputJsonSchema` | `Record<string, unknown>` | No | Explicit JSON Schema override/fallback for non-Zod tool input. Zod uses its native converter; other validators can implement StandardJSONSchemaV1. A validation-only schema must provide this field or conversion throws `MISSING_JSON_SCHEMA` |
...
Standard Schema support covers tool input/output/event schemas and `callModel` / sub-harness structured output only. Channels, context-layer schemas, item extension schemas, and JSON workflow schemas remain Zod-specific.
...
- [callModel](/docs/framework/steps/call-model) -- pass tools to an LLM and let the model decide when to call them.
- [runCode](/docs/framework/steps/run-code) -- for arbitrary async logic that is not a tool.
