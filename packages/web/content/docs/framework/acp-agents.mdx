...
Three things to know about sharing a session:

- **Reuse is scoped to a root run.** A connection owns a live agent process, so the harness closes every session it still holds when the run finishes. `'keep'` means "for the rest of this run", not "forever" — a session outliving its run would stop your process from exiting. Steps sharing a session must therefore be part of one run (a `sequence`, a `loop`, or nested `ctx.harness.run` calls), not separate `harness.execute()` calls.
- **Each step gets its own policy.** The permission policy, steering hook, handler, and event stream are rebound before every turn, so the "investigate read-only, then fix with edits" pattern really works: the second step's broader permissions do apply, even though the first step opened the connection.
