...
| `subprocess` | `SubprocessAdapter` | `createInMemorySubprocessAdapter()` | Adapter used to dispatch every `step.runCode`, `spawn`, and `detachedSpawn`. In-memory by default; set to `createLocalSubprocessAdapter({storage})` to run children out-of-process with durable handle manifests. See [Durability](/docs/framework/durability). |
...
The harness always holds a `SubprocessAdapter`. Every `step.runCode`, every `spawn`, and every `harness.detachedSpawn` routes through `harness.subprocess.spawn(...)` — in-process vs out-of-process is a property of the adapter, not the step.
