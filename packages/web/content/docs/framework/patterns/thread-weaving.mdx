...
```ts validate=none
import { fork, react } from '@noetic-tools/core';
...
const multiSourceResearch = fork({
  ...
  mode: 'all',
  paths: () => [
    react({ model: 'openai/gpt-4o', instructions: 'Search academic papers.', tools: [scholarTool] }),
    react({ model: 'openai/gpt-4o', instructions: 'Search news articles.', tools: [newsTool] }),
    react({ model: 'openai/gpt-4o', instructions: 'Search code repositories.', tools: [githubTool] }),
  ],
  merge: (results, ctx) => {
...
```ts validate=none
import { fork } from '@noetic-tools/core';
