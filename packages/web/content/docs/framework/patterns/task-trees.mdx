...
```ts
import { compilePlan, react } from '@noetic-tools/core';
...
```ts
import { PlanNodeSchema } from '@noetic-tools/core';
...
- [Adaptive Plans](/docs/framework/patterns/adaptive-plans) -- dynamic plans that revise themselves on failure
- [ReAct](/docs/framework/patterns/react) -- the agent pattern used inside each tree node
