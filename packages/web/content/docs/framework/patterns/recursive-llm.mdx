...
## Controlling Child Context with Context Layers

Context layers give you fine-grained control over what the child sees and how results return to the parent.
...
- Attach a context layer with an `onReturn` hook to compress child results before they flow back to the parent. This keeps the parent's context from growing unboundedly.
