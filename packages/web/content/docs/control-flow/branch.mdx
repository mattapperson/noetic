...
| `route` | `(input: I, ctx: Context) => Step<I, O> \| null \| Promise<Step<I, O> \| null>` | Yes | Function that picks the step to run (sync or async) |
...
The `route` function can be async, enabling AI-powered and embedding-based routing. Noetic provides condition helpers that return async route functions:

### `semanticSwitch` -- Embedding-Based Routing

Routes input to the most semantically similar case using embeddings:
...
### `semanticRoute` -- Condition-Based Routing
...
Combine conditions with `anyCondition` (OR) and `allCondition` (AND):

```ts
import { anyCondition, allCondition, embeddingMatch, aiCondition } from '@noetic/core';
...
All embedding conditions accept an optional `cache: StorageAdapter` to persist label embeddings across ephemeral invocations (e.g., Cloudflare Workers):

```ts
const route = semanticSwitch({
  embed,
  cases: { /* ... */ },
  cache: myStorageAdapter, // persists label vectors
});
```
