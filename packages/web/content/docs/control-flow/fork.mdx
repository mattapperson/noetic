...
```ts
import type { SettleResult } from '@orchid/core';
...
| `error` | `OrchidError \| undefined` | The error (present when rejected) |
...
| `paths` | `(input: I, ctx: Context) => Step<I, O>[]` | Yes | Function returning the steps to run |
...
| --- | --- | --- | --- |
| `merge` | `all` | `(results: O[], ctx: Context) => O` | Combine all outputs into one |
| `merge` | `settle` | `(results: SettleResult<O>[], ctx: Context) => O` | Combine settled results into one |
...
- [branch](/docs/control-flow/branch) -- conditional routing for single-path decisions.
- [Spawn](/docs/spawn) -- isolated child contexts for deeper isolation.
