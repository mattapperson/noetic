...
Noetic provides a set of condition helpers that let you route pipeline inputs using embeddings, LLM classifiers, and boolean combinators. These helpers produce async route functions compatible with [`branch`](/docs/control-flow/branch).
...
- [branch](/docs/control-flow/branch) -- the control flow primitive that uses route functions.
- [fork](/docs/control-flow/fork) -- parallel execution.
- [Adapters](/docs/api/adapter-types) -- `EmbedFn` and `CallModelFn` types.
