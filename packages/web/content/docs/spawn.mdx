...
For background sub-agents that run concurrently, use `runtime.detachedSpawn()`. The parent continues immediately and receives a `DetachedHandle` to track the child.
...
Combine detached spawn with the [loop inbox channel](/docs/loop-and-until#inbox-channel) for a full async delegation workflow:
...
const handle = runtime.detachedSpawn(subAgentStep, task, ctx);
handle.await().then((result) => {
  runtime.send(inbox, `[Sub-agent done] ${result}`, ctx);
});
...
```ts
import { channel, react, spawn, step, until } from '@noetic/core';
import { z } from 'zod';
...
const delegateTool = {
  ...
  execute: async (args: { task: string }) => {
    ...
    return runtime.execute(spawn({ id: 'sync-spawn', child }), args.task, ctx);
  },
};
...
const launchTool = {
  ...
  execute: async (args: { task: string }) => {
    ...
    const ctx = runtime.createContext();
    const handle = runtime.detachedSpawn(child, args.task, ctx);
    // Notify the parent loop when the sub-agent finishes
    handle.await().then(
      (result) => runtime.send(inbox, `[Agent ${handle.id} done] ${result}`, ctx),
      (err) => runtime.send(inbox, `[Agent ${handle.id} failed] ${err}`, ctx),
    );
