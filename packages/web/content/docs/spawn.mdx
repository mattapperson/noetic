...
const handle = runtime.detachedSpawn(subAgentStep, task, parentCtx);
handle.await().then((result) => {
  runtime.send(inbox, `[Sub-agent done] ${result}`, parentCtx);
});
...
const delegateTool = tool({
  ...
  execute: async (args: { task: string }, parentCtx: Context) => {
    ...
    return runtime.execute(spawn({ id: 'sync-spawn', child }), args.task, parentCtx);
  },
});
...
const launchTool = tool({
  ...
  execute: async (args: { task: string }, parentCtx: Context) => {
    ...
    const handle = runtime.detachedSpawn(child, args.task, parentCtx);
    // Notify the parent loop when the sub-agent finishes
    handle.await().then(
      (result) => runtime.send(inbox, `[Agent ${handle.id} done] ${result}`, parentCtx),
      (err) => runtime.send(inbox, `[Agent ${handle.id} failed] ${err}`, parentCtx),
    );
