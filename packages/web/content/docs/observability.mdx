...
Orchid includes a built-in tracing system modeled on OpenTelemetry conventions. Every step execution, LLM call, and tool invocation creates a span with timing data, token counts, and custom attributes. You can export these spans to any observability backend.
...
Every [Context](/docs/context) has a `span` property. You can add custom attributes from within any step:
...
```ts
import { NoopExporter } from '@orchid/core';
...
```ts
import { InMemoryExporter } from '@orchid/core';
...
```ts
import type { TraceExporter, Span } from '@orchid/core';
...
Orchid defines constants following the OpenTelemetry GenAI semantic conventions. These are automatically set on LLM and tool spans:
...
```ts
import { GenAI, ToolAttr } from '@orchid/core';
...
- [Context & Event Log](/docs/context) -- every context carries a `span`.
- [Runtime](/docs/runtime) -- `createSpan` and trace exporter configuration.
- [Memory](/docs/memory) -- memory layer operations produce `MemoryTraceSpan` records.
