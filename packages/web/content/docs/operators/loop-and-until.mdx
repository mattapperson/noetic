...
| `steps` | `ReadonlyArray<Step<I, O>>` | Yes | -- | Steps to execute sequentially on each iteration |
...
See [Detached Spawn](/docs/operators/spawn#detached-spawn) for the full async delegation pattern.
...
- [Steps](/docs/steps) -- the step types you can use in a loop.
- [Spawn](/docs/operators/spawn) -- run a loop in an isolated child context.
- [Control Flow](/docs/operators) -- branch and fork for non-iterative operations.
