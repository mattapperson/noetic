...
Noetic provides a set of condition helpers that let you route pipeline inputs using embeddings, LLM classifiers, and boolean combinators. These helpers produce async route functions compatible with [`branch`](/docs/operators/branch).
...
- [branch](/docs/operators/branch) -- the control flow primitive that uses route functions.
- [fork](/docs/operators/fork) -- parallel execution.
- [Adapters](/docs/api/adapter-types) -- `EmbedFn` type and `LlmProviderConfig`.
