 ...
 *
 * Uses reactive lastMessage from the WebSocket store to ensure messages
 * are received reliably, including after Next.js HMR module reloads.
 */
...
export function useExecutionMessages(): void {
  ...
  const lastProcessedRef = useRef<ServerMessage | null>(null);

  useEffect(() => {
    if (!lastMessage) {
      return;
    }
    // Skip if we already processed this exact message reference
    if (lastMessage === lastProcessedRef.current) {
      return;
    }
    lastProcessedRef.current = lastMessage;

    const message = lastMessage;
    const { addAgent, addRun, updateRun } = useAgentStore.getState();
    ...
    switch (message.type) {
      ...
      case 'execution.list.response': {
        const { addAgent: addAgentAction } = useAgentStore.getState();
