 ...
 *
 * This module keeps `node:net` and `node:fs` imports — it is the Node
 * implementation of the transport. The CLI and any Node-runtime
 * consumer imports from here; non-Node consumers would implement an
 * alternative transport against the `agent-ipc-protocol` wire shape.
 */
...
import type { PendingAskUserRequest } from '../ask-user-service.js';
