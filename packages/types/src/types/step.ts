 ...
 * the step boundary.
 *
 * The interpreter resolves `runtime` at dispatch (same `Lazy` convention as
 * other step fields), applies `inputSchema`/`output` Standard Schema
 * validation on both edges of the program, wraps the run in `retry`, and maps
 * a rejected `NoeticError` of kind `cancelled` — fiber interruption — onto
 * the framework's cancellation surface (spec 09).
 * @public
 */
