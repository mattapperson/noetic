...
function buildLayerHandle(layer: ContextLayer, ctx: Context): Readonly<Record<string, unknown>> {
...
/**
 * Build the `ctx.context` object — a readonly map of layer ID → resolved handle.
 * Handles use getters for data (live reads) and closures for functions (lazy state).
 */
export function buildContextData(layers: ReadonlyArray<ContextLayer>, ctx: Context): ContextData {
  const data: Record<string, Record<string, unknown>> = {};

  for (const layer of layers) {
    data[layer.id] = buildLayerHandle(layer, ctx);
  }

  return frameworkCast<ContextData>(data);
}
