...
- [Routing Between Agent Types](#routing-between-agent-types)
...
All execution — the coordinator loop, every routed agent step, and any spawned sub-agents — shares one `AgentHarness`. The harness provides the LLM client, channel store, tracing, and storage.
...
const agentRouter = conditional({
  id: 'agent-type-router',
  route: (input, ctx) => {
    // Read which agent type to use from the scratchpad or input
...
Plan mode restricts which agent types are available. In Noetic, this is just another layer of conditional routing — the `route` function checks the mode and filters agent types.

### Conditional-Based Mode Restriction
...
```typescript
import {
  AgentHarness, loop, step, conditional,
...
Iteration 1:
  ...
  agentRouter → exploreStep (haiku, read-only tools) → runs, returns file list

Iteration 2:
  coordinatorStep → decides: { agentType: 'plan', task: 'design dark mode toggle' }
  agentRouter → planStep (sonnet, read-only tools) → runs, returns plan

Iteration 3:
  coordinatorStep → calls set_mode({ mode: 'act' }), decides: { agentType: 'general', task: 'implement plan' }
  agentRouter → generalStep (sonnet, ALL tools including write/edit/bash) → implements

Iteration 4:
  coordinatorStep → decides: { agentType: 'verification', task: 'verify implementation' }
  agentRouter → verifyStep (sonnet, read-only + /tmp) → returns VERDICT: PASS

Iteration 5:
  coordinatorStep → { done: true } → until.noToolCalls() fires → loop ends
```

Each iteration, the model only sees the tools for its agent type. The explore step has no write tools. The general step has everything. The router handles this — no steering needed, no dynamic step construction.

---

## Async Sub-Agents

The loop-with-router approach above is sequential — one agent type per iteration. For parallel sub-agents (Claude Code's async delegation), you need `spawn` + `detachedSpawn` + an inbox channel. This is an extension of the base pattern, not a replacement.
...
### The Noetic Equivalent: The `observations()` Layer

`observations()` buffers raw conversation items. When the buffer crosses a token threshold, it calls an `observer` function to distill them into compact observations. This is a **context layer** — it controls what the model sees in its prompt, not what tools are available:

```typescript
import { observations } from '@noetic-tools/core';

const compactor = observations({
...
```typescript
import {
  AgentHarness, step, conditional, loop, spawn, tool, channel,
  scratchpad, observations, steering, SteeringAction,
} from '@noetic-tools/core';
import { any, until } from '@noetic-tools/core';
import { z } from 'zod';
...
Iteration 1:
  ...
  router → exploreStep (haiku, read-only tools)
  → "Found SettingsPage.tsx, ThemeContext.tsx..."

Iteration 2:
  coordinator → select_agent({ type: 'plan' })
  router → planStep (sonnet, read-only tools)
