...
All agent patterns compose through a single `Step<I, O>` type. Steps are pure data (no side effects until executed). The runtime dispatches by `step.kind`:
...
Tools are defined with Zod schemas for input/output validation. Inside `execute`, tools receive `ToolExecutionContext` which provides `runtime`, `ctx`, and `memory` accessors:
...
const myTool = tool({
  ...
  execute: async (args, toolCtx) => {
    // toolCtx.runtime -- guaranteed non-undefined Runtime
...
Recall can return a `RecallResult` object or a plain `string` (shorthand -- the runtime wraps it in a developer message).
...
Define tools with Zod schemas. Tools that spawn sub-agents should use `toolCtx.runtime.execute()` or `toolCtx.runtime.detachedSpawn()` -- never capture runtime in a closure.
...
For complete builder signatures, memory layer APIs, runtime methods, and slot constants, read `references/api-reference.md`.
...
| Runtime | `packages/core/src/runtime/in-memory-runtime.ts` |
