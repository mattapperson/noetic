...
| `32-system-one-decisions.md` | `packages/types/src/types/system-one.ts`, `interpreter/execute-decide`, `builders/step-builders` (`step.decide`), `packages/types/src/schemas/workflow.ts` + `builders/workflow-hydrator.ts` (`decide` node, `conditional` predicate union), `runtime/durable/step-ledger.ts` (`decide` content hash), `packages/system-one/src/`, `packages/context/src/context/layers/decision-compaction.ts` | `framework/system-one-decisions.mdx`, `framework/context-layers/decision-compaction.mdx` (+ run `bun run gen:schema`, see Requirement 6) |
